...
CREATE VIEW vw_vliente_detalhes AS 
...
/*11. Crie uma VIEW chamada "vw_clientes_aniversariantes" que exibe o nome do cliente, telefone, e mês de nascimento, 
para clientes que fazem aniversário no primeiro semestre (janeiro a junho).*/
CREATE VIEW vw_clientes_aniversariantes AS 
SELECT nome_cliente, telefone_cliente,
	MONTH (data_nascimento) AS ano_nascimento
FROM tb_cliente
WHERE MONTH(data_nascimento) BETWEEN 1 AND 6;    

/*12. Crie uma VIEW chamada "vw_produtos_lucro" que exibe o nome do produto, categoria e o lucro bruto de cada produto 
(preco_venda - preco_compra) apenas para os produtos com estoque superior a 50 unidades.*/
...
/*13. Crie uma VIEW chamada "vw_clientes_mais_lucrativos" que exibe o nome do cliente, telefone, cidade, estado e a soma do lucro 
bruto de todos os produtos com estoque menor que 50 vendidos para clientes que moram em estados do Sudeste (SP, RJ, MG, ES).*/
...
SELECT tb_cliente.nome_cliente, tb_cliente.telefone_cliente, tb_cliente.cidade, tb_cliente.estado,
	SUM(tb_produto.preco_venda - tb_produto.preco_compra) AS soma_lucro_bruto
FROM tb_cliente 
JOIN tb_produto
WHERE estoque < 50 AND estoque IN ('SP', 'RJ', 'MG', 'ES')
AND tb_produto.estoque < 50
GROUP BY tb_cliente.nome_cliente, tb_cliente.telefone_cliente, tb_cliente.cidade, tb_cliente.estado;

/*14. Usando a view "vw_clientes_aniversariantes", selecione os nomes dos clientes que fazem aniversário em um mês ímpar e 
ordene pelo mês de nascimento. Utilize uma condição para exibir o mês por extenso (ex: 'Janeiro', 'Março', etc.).*/
...
WHERE data_nascimento IN (1, 3, 5)
ORDER BY data_nascimento;

/*15. Usando as views "vw_produtos_lucro" e "vw_clientes_mais_lucrativos", selecione os nomes dos clientes e o lucro total de 
produtos da categoria 'Alimentos' vendidos para esses clientes, mas apenas para aqueles que geraram lucro acima de 100. 
Ordene pelo lucro em ordem decrescente.*/

SELECT nome_cliente, 
	SUM(lucro_bruto) AS lucro_bruto
FROM vw_clientes_mais_lucrativos
JOIN vw_produtos_lucro ON nome_cliente = nome_cliente
WHERE categoria = 'Alimentos'
GROUP BY nome_cliente
HAVING SUM(lucro_bruto) > 100
